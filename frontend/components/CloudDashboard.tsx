...
export default function CloudDashboard() {
  ...
  const refresh = useCallback(async (wallet = account) => {
    try { const state = await api<Snapshot>(`/api/state${wallet ? `?wallet=${encodeURIComponent(wallet)}` : ''}`); setData(state); setNotice('');
      const saved = localStorage.getItem(KEY); if (saved) { const { id, wallet: owner } = JSON.parse(saved); if (same(owner, wallet)) {
        const found = state.requests.find(item => item.id === id); if (found) setActive(found);
      } }
  ...
  useEffect(() => {
    ...
    if (ethereum) ethereum.request({ method: 'eth_accounts' }).then(value => { const first = (value as string[])[0] || ''; setAccount(first); void refresh(first); }).catch(() => void refresh(''));
    else void refresh('');
    const timer = setInterval(() => { if (document.visibilityState === 'visible') void refresh(); }, 12_000);
    return () => clearInterval(timer);
  }, [refresh]);

  async function connect() {
    if (!window.ethereum) { setNotice('Install MetaMask to sign Polygon Amoy transactions.'); return; }
    setBusy(true);
    try {
      await window.ethereum.request({ method: 'wallet_switchEthereumChain', params: [{ chainId: AMOY }] });
    } catch (error) {
      if ((error as { code?: number }).code !== 4902) { setNotice(errorText(error)); setBusy(false); return; }
      await window.ethereum.request({ method: 'wallet_addEthereumChain', params: [{ chainId: AMOY, chainName: 'Polygon Amoy',
        nativeCurrency: { name: 'Test POL', symbol: 'POL', decimals: 18 }, rpcUrls: ['https://rpc-amoy.polygon.technology'],
        blockExplorerUrls: ['https://amoy.polygonscan.com'] }] });
    }
    try { const accounts = await window.ethereum.request({ method: 'eth_requestAccounts' }) as string[]; setAccount(accounts[0]); setNotice(''); await refresh(accounts[0]); }
    catch (error) { setNotice(errorText(error)); }
    finally { setBusy(false); }
  }

  async function run(action: Action) {
    if (!account || !window.ethereum || lock.current) return;
    const id = crypto.randomUUID();
    localStorage.setItem(KEY, JSON.stringify({ id, wallet: account, action }));
    lock.current = true; setBusy(true); setModal(null);
    setActive({ id, action, status: 'preparing', message: 'Saved in this browser. The backend is preparing the exact wallet transaction.' });
    try {
      const prepared = await post<Prepared>('/api/requests', { id, wallet: account, action });
      setActive(prepared.record);
      const hash = await window.ethereum.request({ method: 'eth_sendTransaction', params: [{ ...prepared.transaction, from: account }] }) as string;
      const record = await post<RequestRecord>(`/api/requests/${id}/transaction`, { wallet: account, hash });
      setActive(record); await refresh(account);
    } catch (error) {
      if ((error as { code?: number }).code === 4001) {
        try { setActive(await post<RequestRecord>(`/api/requests/${id}/reject`, { wallet: account })); } catch { setActive(prev => prev && ({ ...prev, status: 'wallet_rejected', message: 'Wallet request cancelled.' })); }
      } else setActive(prev => prev && ({ ...prev, status: 'unknown', message: `${errorText(error)} Check this request before trying again.` }));
  ...
  async function check(record = active) {
    ...
    try { setActive(await api<RequestRecord>(`/api/requests/${record.id}?wallet=${encodeURIComponent(account)}`)); await refresh(account); }
  ...
  return <div className="cl-shell"><aside className="cl-sidebar">
    ...
    <div className="cl-sidebar-note"><ShieldCheck/><strong>Wallet signed. Publicly verifiable.</strong><p>The backend stores exact evidence bytes. Polygon stores their fingerprints and escrow state.</p></div><small>Polygon Amoy testnet · v3</small>
